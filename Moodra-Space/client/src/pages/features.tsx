  ...
  en: {
    ...
    sections: [
      ...
      },
      {
        ...
        subtitle: "50/50 split workspace: write experimental drafts on the left, manage author role models on the right — all in one panel.",
        items: [
          "Drafts panel — full block editor with font scale, max-width, and autosave on every keystroke",
          "Move any draft to your book as a new chapter or append to an existing one",
          "Role Models — upload epub, fb2, or txt files and get a deep AI style analysis instantly",
          "File upload + custom instruction: tell the AI what to focus on in the analysis",
          "Each role model gets an influence slider (0–100%) — styles blend proportionally when AI writes",
          "Drafts toolbar matches the chapter editor: font scale, width, typewriter mode, word sprint",
          "Canvas mode — all chapters as one continuous scroll with full editing capabilities",
        ],
      },
      {
        icon: Brain,
  ...
  ru: {
    ...
    sections: [
      ...
      },
      {
        ...
        subtitle: "Трёхблочная студия, которая держит все ваши исследования рядом с книгой — никакого переключения инструментов.",
        items: [
          "AI-поиск источников — опишите, что нужно, получите структурированные результаты: авторы, ключевые мысли, цитаты, оценка релевантности",
          "Сохраните любой результат в Библиотеку источников одним кликом",
          "Панель черновиков — пишите и редактируйте исследовательские черновики в том же блочном редакторе, что и основная книга",
          "Автосохранение при каждом изменении — ни один черновик не потеряется",
          "Перемещение черновика в книгу: новой главой или добавлением к существующей",
          "Ролевые модели — добавляйте анализы авторов, смотрите полные профили стиля с раскрываемыми секциями",
          "Режим полотна — все главы как одно непрерывное полотно с полноценными функциями редактирования",
        ],
      },
      {
        icon: Brain,
